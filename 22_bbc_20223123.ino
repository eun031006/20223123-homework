...
// Servo adjustment
#define _DUTY_MAX 1740  // Servo angle: D degree
#define _DUTY_NEU 1500  // Servo angle: 0 degree
#define _DUTY_MIN 1290   // Servo angle: E degree
...
void loop() {
  ...
  if(event_dist) {
    ...
    if(dist_target > dist_ema) { // Replace 1 with a proper test expression using dist_target and dist_ema.
      duty_target = duty_target + _BANGBANG_RANGE; // Complete this line using _BANGBANG_RANGE.
...
// Replace the below line with the equation obtained from nonlinear regression analysis
  return 511 - 1.69*a_value + 1.49E-03*a_value*a_value;
}
